...
FASHION_MNIST_DIR = os.path.join(DATA_FOLDER, "FASHION_MNIST")
# easy to find!
...
def fashion_mnist(
    folder=FASHION_MNIST_DIR, one_hot=True, partitions=None, filters=None, maps=None, shuffle=False
):
    datasets = input_data.read_data_sets(folder,one_hot=one_hot)
    train = dl.Dataset(datasets.train.images, datasets.train.labels, name="FASHION_MNIST")
    validation = dl.Dataset(
        datasets.validation.images, datasets.validation.labels, name="FASHION_MNIST"
    )
    test = dl.Dataset(datasets.test.images, datasets.test.labels, name="FASHION_MNIST")
    res = [train, validation, test]
    if partitions:
        res = redivide_data(
            res,
...
'''
def fashion_mnist(
    folder=None, partitions=None, filters=None, maps=None, shuffle=False
):
    if not folder:
        folder = CIFAR10_DIR
    (x_train,y_train), (x_test,y_test) = cifar10.load_data()
    x_train = np.reshape(x_train,(-1,3072))
    y_train_new = [y_train[label][0] for label in range(len(y_train))]
    y_train = dl.to_one_hot_enc(y_train_new,dimension=10)
    x_test = np.reshape(x_test,(-1,3072))
    y_test_new = [y_train[label][0] for label in range(len(y_test))]
    y_test = dl.to_one_hot_enc(y_test_new,dimension=10)
    train = dl.Dataset(x_train, y_train, name="CIFAR_train")

    test = dl.Dataset(x_test, y_test, name="CIFAR_test")
    res = [train, test]
    if partitions:
        res = redivide_data(
            res,
